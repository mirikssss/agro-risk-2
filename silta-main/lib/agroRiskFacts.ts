...
export type Readiness = "ready" | "in_progress" | "planned";
...
export type ProductFacts = {
  ...
  currentStatus: {
    mvp: Readiness;
    mvpNotes: string[];
    stakeholderConversations: { org: string; status: Readiness; notes: string[] }[];
    dataset: { satelliteYears: number; coverageNote: string; status: Readiness; notes: string[] };
  };
  problem: { corePain: string; whyNow: string[]; commonBankPainPoints: string[] };
  solution: { whatWeDeliver: string[]; outcomes: string[]; whyItMatters: string[] };
  howItWorks: {
    steps: Array<{
      id: 1 | 2 | 3 | 4;
      title: string;
  ...
  dataAndSignals: {
    satellites: Array<{ name: string; whatItAdds: string[]; exampleSignals: string[] }>;
    weather: { sources: string[]; features: string[]; notes: string[] };
    soilAndLand: { note: string; proxies: string[]; whyUseful: string[] };
    fieldBoundaries: { status: Readiness; whyImportant: string[]; approach: string[]; disclaimer: string[] };
  };
  model: {
    philosophy: string[];
    outputs: {
      riskScore: string;
      trend: string;
      scenarios: { p10: string; p50: string; p90: string; plainExplanation: string[] };
      drivers: string[];
      alerts: string[];
    };
    explainability: { whatWeShow: string[]; howWeAvoidBlackBox: string[] };
    benchmarks: {
      mae_p50: number;
      rmse_p50: number;
      coverage_p10_p90_pct: number;
      downside_miss_rate_pct: number;
      volatility_spread_mean: number;
      summary: string;
      notes: string[];
    };
  };
  recommendations: { engine: string; types: string[]; format: string[]; disclaimer: string[] };
  differentiators: { vsTypicalProcess: string[]; vsGenericCompetitors: string[]; defensibility: string[] };
  businessModel: {
    primaryUsers: string[];
    offerings: Array<{ name: string; bestFor: string[]; pricingLogic: string[]; integration: string[] }>;
  };
  roadmap: {
    phases: Array<{
      title: string;
      timeframe: string;
      readiness: Readiness;
      checklist: Array<{ item: string; status: "done" | "in_progress" | "planned" }>;
    }>;
    wowButReal: string[];
  };
  complianceAndBoundaries: { truthRules: string[]; partnerships: string[]; privacy: string[] };
  faqSeeds: Array<{ q: string; intentTag: string }>;
};

export const FACTS: ProductFacts = {
  product: {
    name: "AgroRisk",
    tagline: "Satellite-to-score risk intelligence for banks & insurers",
    oneLiner:
      "We turn satellite + weather signals into bank-ready risk scores, early warnings, and actionable recommendations — weeks before losses materialize.",
    websitePositioning:
      "From Sentinel signals to portfolio decisions — with clear p10/p50/p90 scenarios and explainable drivers.",
  ...
  currentStatus: {
    ...
    dataset: {
      satelliteYears: 8,
      coverageNote:
        "We have assembled multi-year satellite signal history suitable for training/validation and seasonal baselines.",
      status: "ready",
      notes: [
  ...
  problem: {
    corePain:
      "Banks and insurers discover agricultural risk too late — after crop stress becomes default, not when early signals appear.",
    whyNow: [
  ...
  howItWorks: {
    pipelineSummary:
      "Field/Region → Satellite & Weather Signals → ML Risk Outputs (p10/p50/p90) → Alerts → Explainable Insights → AI Recommendations → Decision/API",
    steps: [
      {
        id: 1,
  ...
  model: {
    ...
    outputs: {
      riskScore:
        "A calibrated risk score designed for credit workflows (interpretable scale; higher means higher risk).",
      trend:
        "Direction of change over recent windows (improving / stable / worsening) to support monitoring.",
      scenarios: {
    ...
    benchmarks: {
      ...
      summary:
        "Backtested yield risk engine: p50 MAE 1.3464, RMSE 1.6038. Uncertainty band p10–p90 achieves 80.01% coverage (within 70–85% target). Downside miss rate 16.89% (within 10–20% target). Mean p90–p10 spread 2.6196 for risk-sensitive decisioning.",
      notes: [
  ...
  recommendations: {
    engine:
      "An AI guidance layer that translates model outputs + historical patterns into banker-friendly actions (LLM-assisted, grounded in computed signals).",
    types: [
